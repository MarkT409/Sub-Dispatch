...
import { findAdminByPhone } from "@/lib/admin-phone-auth";
import {
  createPhoneOtp,
  formatPhoneDisplay,
  isTwilioConfigured,
...
export async function POST(request: Request) {
  ...
  const match = await findAdminByPhone(supabase, raw);

  if (!match) {
    return NextResponse.json(
      {
        error:
          "That number isn’t authorized for admin access. Ask a supervisor to add your phone under Users (or ADMIN_PHONES).",
      },
      { status: 404 },
    );
  }
  ...
  try {
    ...
    return NextResponse.json({
      ok: true,
      phoneDisplay: formatPhoneDisplay(raw),
      email: match.email,
      smsSent: sms.sent,
      ...(allowDevCode ? { devCode: code } : {}),
    });
  } catch (err) {
    console.error("admin phone OTP start failed:", err);
    return NextResponse.json(
